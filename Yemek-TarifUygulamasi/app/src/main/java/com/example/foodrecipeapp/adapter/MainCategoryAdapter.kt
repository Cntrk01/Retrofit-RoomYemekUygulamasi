...
class MainCategoryAdapter: RecyclerView.Adapter<MainCategoryAdapter.RecipeViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecipeViewHolder, position: Int) {
        holder.itemView.tv_dish_name.text=arrMainCategory.get(position).strcategory
        Glide.with(ctx!!).load(arrMainCategory.get(position).strcategorythumb).into(holder.itemView.img_dish)
        holder.itemView.rootView.setOnClickListener {
            listener!!.OnClicked(arrMainCategory.get(position).strcategory)
        }
    }
